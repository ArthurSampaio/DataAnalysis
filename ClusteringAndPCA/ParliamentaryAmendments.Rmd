...
Para escolher um bom parâmetro k é necessário observa a distância entre os clusters que pode ser visualizado no eixo y. Entre y = 0 e y = 5, há 4 subclusters que possui poucas distâncias entre si. Assim, k = 4.

```{r}
    plot(emendas.area2.hclust, hang = -1, labels = FALSE)
    rect.hclust(emendas.area2.hclust, k = 4)

```

Após os dados estarem agrupados, basta plotar o gráfico. 

```{r}
  emendas.area.agrup = cutree(emendas.area2.hclust, k = 3)
  ggplot(emendas.area, aes(x = Agricultura, y = Ciência.e.Tecnologia)) +
     geom_point(aes(color = factor(emendas.area.agrup), alpha = 0.1))
```
...
#LIXO?
```{r}
  emendas.area2 = emendas.area %>% select(Agricultura, Assistência.Social,Ciência.e.Tecnologia,Comércio.e.Serviços,
                         ...
                         Saúde, Segurança.Pública, Trabalho, Urbanismo) %>% dist() %>%
