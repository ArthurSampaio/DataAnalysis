...
Os valores estão muito concentrados a esquerda do gráfico, além disto os valores crescem exponencialmente. Para facilitar a visualização é plotado um gráfico monolog. É importante frisar que ao plotar um gráfico monolog os valores negativos, referentes a compensação de débitos não serão mostrados. 
```{r}
...
ggplot(gastosDeputadosPB) + 
  geom_density(kernel = "gaussian", mapping = aes(x = ValorGasto))

```


```{r}
    ggplot(gastosDeputadosPB, mapping = aes(x = GastoTotal/1e3, 
                             y = ValorGasto/1e3)) + 
      geom_boxplot() + 
...
Como os valores variam exponencialmente, é plotado um gráfico monolog para facilitar a visualização. 
